    ...
    for i in sheet_names:
        ...
        for value in data:
            ...
            if isinstance(value[0], int):
                ...
                # print(excel_data)
                assert_Res = yield excel_data   #返回断言结果
                #成功则回填Pass，失败则Failed
                if assert_Res == 'Pass':
                    sheet.cell(value[0] + 2, 6).value = "Pass"
                    sheet.cell(value[0] + 2, 6).font = Font(name='仿宋', size=12, color=Color(index=3), b=True)
                elif assert_Res == 'Failed':
                    sheet.cell(value[0] + 2, 6).value = "Failed"
                    sheet.cell(value[0] + 2, 6).font = Font(name='仿宋', size=12, color=Color(index=2), b=True)
